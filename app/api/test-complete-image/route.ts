/**
 * Endpoint de prueba para generación de imagen completa con DALL-E 3
 * Fondo fotográfico + Texto overlay en una sola generación
 */

import { NextResponse } from 'next/server';
import { db } from '@/lib/firebase';
import { generateCompleteImageWithRetry } from '@/lib/services/complete-image-generator';

export async function GET() {
  try {
    console.log('🧪 Probando generación de imagen completa con DALL-E 3...');
    ...
    const result = await generateCompleteImageWithRetry({
      titulo: data.titulo,
    ...
    console.error('❌ Error en test de imagen completa:', error);
    return NextResponse.json({
      success: false,
